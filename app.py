...
def main():
    #try:
      st.title("Image Captioning Through Deep Learning")
      st.sidebar.title("Image Captioning Through Deep Learning")
      ...
      choice = st.radio("Select a choice",("Generate-Caption","Fetch Similar Images","Text-To-Speech"))
      if choice == "Generate-Caption":
      ...
      elif choice == "Fetch Similar Images":
        ...
        input = [input]
        st.write(input)
        option = st.sidebar.selectbox('Select Similarity Measure',["Cosine-Similarity","Pearon-Correlation"])
        if option=="Cosine-Similarity":
          y=tfidf.transform(input).toarray()
          cosine_sim = cosine_similarity(features,y)
          pd_dataset["similarity"]=cosine_sim
          #df=pd_dataset.sort_values(by=['similarity'],ascending=False)
          #st.write(df)
          fetch = st.sidebar.button('Fetch Images',key='fetch')
          if fetch:
             df=pd_dataset.sort_values(by=['similarity'],ascending=False)
             st.write(df)
